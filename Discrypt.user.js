// ==UserScript==
// @name         Discrypt
// @namespace    https://github.com/egonaut/Discrypt
// @version      1.0
// @description  Live Encryption/Decryption of Discord Messages
// @author       egonaut
// @license      LGPLv3 - https://www.gnu.org/licenses/lgpl-3.0.txt
// @downloadURL  https://github.com/egonaut/Discrypt/raw/master/Discrypt.user.js
// @updateURL    https://github.com/egonaut/Discrypt/raw/master/Discrypt.meta.js
// @icon         https://github.com/egonaut/Discrypt/raw/master/images/logo.png
...
const BlacklistUrl = "https://github.com/egonaut/Discrypt/raw/master/blacklist.txt";
...
var Discord;
var Utils = {
    Log: (message) => { console.log(`%c[Discrypt] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
    Warn: (message) => { console.warn(`%c[Discrypt] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
    Error: (message) => { console.error(`%c[Discrypt] %c${message}`, `color:${BaseColor};font-weight:bold`, "") }
};
var DataBase;
var Cache;
var Blacklist;
...
function Init(nonInvasive)
{
    ...
    Object.assign(Utils, {
        ...
        LoadDb: function(callback, failCallback, reload) { (async () => {
            if(!reload) DataBase = await this.StorageLoad('Discrypt');
            if(DataBase != null) {
                Cache = { keys: {} };
        ...
        SaveDb: async function() {
            ...
            await this.StorageSave('Discrypt', DataBase);
        },
        saveDbTimeout: null,
        FastSaveDb: function() {
            this.dbChanged = true;
        ...
        DownloadDb: async function(uncompressed) {
            ...
            this.DownloadBlob(uncompressed ? "Discrypt.json" : "Discrypt.dat", new Blob([buffer]));
        },
                ...
                message: {
                    ...
                    embed: {
                        ...
                        author: {
                            ...
                            url: "https://github.com/egonaut/Discrypt"
                        },
                        description: sysmsg,
                        footer: {
                            text: "𝘋𝘪𝘴𝘤𝘳𝘺𝘱𝘵",
                            icon_url: "https://i.ibb.co/Bc1y79J/mine.png",
                        }
                    }
                }
            }, () => { /*TODO*/ });
...
async function handleUpdate(event) {
    ...
    if(message.content == null && message.embeds != null && message.embeds.length === 1) {
        ...
        if(embed.footer != null && (embed.footer.text === "Discrypt" || embed.footer.text === "𝘋𝘪𝘴𝘤𝘳𝘺𝘱𝘵")) {
...
const messageRegex = /^([⠀-⣿]{16,}) `(?:Discrypt|𝘋𝘪𝘴𝘤𝘳𝘺𝘱𝘵)`$/;
...
async function processEmbeds(message, ignoreAttachments) {
    ...
    let embed = message.embeds[0];
    if(embed.footer == null || (embed.footer.text !== "Discrypt" && embed.footer.text !== "𝘋𝘪𝘴𝘤𝘳𝘺𝘱𝘵")) return;
...
async function handleSend(channelId, message, forceSimple) {
    ...
    if(forceSimple || Cache.channelBlacklist === 2 || (channel.type === 0 && !Discord.can(EMBED_LINKS_CHECK, Discord.getCurrentUser(), channel))) {
       message.content = payload + " `𝘋𝘪𝘴𝘤𝘳𝘺𝘱𝘵`";
    }
    else {
        message.content = "";
        message.embed = {
            ...
            author: {
                ...
                icon_url: "https://i.imgur.com/pFuRfDE.png",
                url: "http://github.com/egonaut/Discrypt"
            },
            description: payload,
            footer: {
                text: "𝘋𝘪𝘴𝘤𝘳𝘺𝘱𝘵",
